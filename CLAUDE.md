...
- `Settings.folderSource?: { url, syncIntervalMinutes? }` — optional JSON file on a web server (same format as the folder export, validated by `parseFolders`) that OWNS the folder definitions: every successful fetch **replaces all folders** (then membership is recomputed). While configured, the options Folders tab renders the folders **read-only** (`renderFolderReadOnly`, import hidden, Export kept — it seeds the remote file) and `save()` **skips `saveFolders`** (the file owns them; writing the options page's stale copy could outlive the next "unchanged" fetch).
