# Bookmarks+ — design notes and file map

**Archive, not instruction. Nothing here is loaded into a session.** The full former CLAUDE.md, verbatim: every architecture decision with its reasoning, the annotated file map, linkding API notes and the next-steps list.
...
**Per-folder surface targeting (since 2026-07-10)**
- `Folder.surfaces?: Surface[]` where `Surface = "popup" | "sidebar" | "newtab"` (`shared/types.ts`) — which surfaces a folder appears on. **Absent = everywhere** (the default, today's behavior; keeps exports of unrestricted folders clean and needs no migration). An explicit **empty array `[]` = hidden everywhere** (a way to park a folder without deleting it).
- Filtering is a single pure helper `foldersForSurface(folders, surface)` (`shared/bookmarks.ts`, unit-tested): `f.surfaces === undefined || f.surfaces.includes(surface)`. There is **no runtime "current surface" variable** — each surface is its own webpack entry/bundle, so each passes its own literal id. Applied at **render time inside each surface's `renderFolders`** (`popup.ts`/`sidebar.ts`/`newtab.ts` renamed the param to `allFolders` and filter to a local `folders`), NOT at sync time — `bookmark_ids` are surface-independent and computed once in the background worker. The sidebar/newtab change-detection cache key stays over the **full** folder array (computed earlier in `render()`), so a `surfaces` edit still re-renders.
- **Options UI**: a "Show on" group of three checkboxes (Popup / Sidebar / New tab) in the per-folder `.folder-settings` row (`renderFolderSurfaces` in `options.ts`), all checked by default. **Canonicalise on change**: all three checked → `delete folder.surfaces` (omit = everywhere); any subset → store that array; none → `[]`. Mutates the live folder only, **never `renderTabs()`** (focus theft, same as the Latest/Sort inputs); Save persists via `saveFolders`. The read-only remote view (`renderFolderReadOnly`) adds a `surfaces: …` meta chip when the field is present.
- `parseFolders` (`shared/validation.ts`) validates it against the `SURFACES` set (mirrors `SORT_MODES`) and round-trips it through JSON export/import, the folder JSON editor, and remote folder sources. Covered by unit tests (`foldersForSurface`, `parseFolders` surfaces) + an options driver check (three checkboxes render all-checked; unchecking one persists across a re-render).

**Tag-condition autocomplete (folder editor, since 2026-07-10)**
- A `tag` folder condition's value input has a **fuzzy autocomplete** dropdown of existing tags. Only the `tag` type gets it (`renderTagValueInput` in `options.ts`); `url_contains`/`title_contains` keep the plain `renderConditionValueInput` path, and `provider`/`browser_base` keep their `<select>`s. Free text is always accepted — the dropdown assists, never constrains (a not-yet-existing tag is a valid value).
- **Suggestions are the union of tags across ALL sources** (`allTagCounts()` in `options.ts` — like `providerTagCounts` but no provider-prefix filter, counts summed), because a `tag` condition matches bookmarks provider-agnostically (`matchesNode` checks `tag_names` regardless of source). Candidates are snapshotted once per widget (bookmarks don't change mid-edit — a sync/save re-renders the tab).
- **Ranking** lives in the pure, unit-tested `shared/fuzzy.ts` `fuzzyFilterTags(query, candidates, limit=8)`, a thin wrapper over **fuzzysort** (the 2nd runtime dep): empty/whitespace query → top-N by count (discoverability on focus); otherwise fuzzysort match with an explicit **count-desc tie-break** on equal scores — fuzzysort does NOT preserve input order on ties, so the wrapper re-sorts by `score desc, count desc, tag asc`. Wrapping it keeps `options.ts` library-agnostic and the ranking node-testable. Each match carries `matchedIndexes` (fuzzysort's `result.indexes`) so the widget can bold which characters matched; the pure `highlightRuns(text, indexes)` helper (also in `fuzzy.ts`, unit-tested) merges those into matched/unmatched runs.
- **Widget** (DOM in `options.ts`): input wrapped in `.tag-autocomplete` (`position: relative`) with an absolutely-positioned `ul.tag-suggestions` (hidden until matches). Each suggestion renders its tag with the fuzzy-matched runs wrapped in `<mark class="tag-suggestion-match">` (restyled off `<mark>`'s un-themable yellow to `var(--link)` + bold). Keyboard: ↑/↓ move highlight, Enter fills the input with the highlighted tag, Esc/Tab close; suggestion click uses `mousedown`+`preventDefault()` so the input doesn't blur first. Keeps the live `condition.value = input.value` mutation on every keystroke and **never calls `renderTabs()` on input** (documented focus-theft) — the dropdown manages its own DOM. All suggestion text via `textContent`/`createTextNode` (tags are untrusted). Styles in `options.css` use tokens only. Covered by a `scripts/ui-verify/options.js` driver check (dropdown appears, fuzzy match, matched-char highlight, keyboard select, free-text accepted, non-tag stays plain).
...
shared/
  ...
  bookmarks.ts        — bookmarksToMap, mergeIntoMap, matchesNode (recursive rule
                        ...
                        foldersForSurface (per-folder surface targeting filter)
  ...
  fuzzy.ts            — fuzzyFilterTags() ranked tag suggestions for the folder
                        editor's Tag autocomplete (thin wrapper over fuzzysort;
                        pure/unit-tested; count tie-break on equal scores)
